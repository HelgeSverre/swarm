...
class UI
{
    ...
    protected function drawAgentState(array $status): void
    {
        ...
        if ($operation) {
            ...
            if ($operation === 'classifying' && isset($details['reasoning'])) {
                $reasoning = mb_substr($details['reasoning'], 0, $this->terminalWidth - 20);
                echo $this->drawBoxLine("   → {$reasoning}", ThemeColor::Border, ThemeColor::Muted);
            }

            if ($operation === 'executing_tool' && isset($details['tool_name'])) {
                $progress = isset($details['tool_index'], $details['tool_total'])
                    ? "[{$details['tool_index']}/{$details['tool_total']}]"
                    : '';
            ...
            echo $this->drawBoxLine('💤 Agent is idle', ThemeColor::Border, ThemeColor::Muted);
        }
    }
    ...
    protected function drawDecisionFlow(string $currentOp, string $phase): void
    {
        $flow = [
            ...
            'executing_tool' => ['icon' => '🔧', 'label' => 'Tool'],
        ...
        foreach ($flow as $op => $info) {
            ...
            if ($op !== 'generating_summary') {
                $flowLine .= $this->colorize(' → ', ThemeColor::Muted);
            }
        }
    ...
    protected function formatOperationMessage(string $operation, string $phase, array $details): string
    {
        $baseMessage = match ($operation) {
            ...
            'extracting_tasks' => 'Identifying tasks',
            'planning_task' => 'Creating execution plan',
        ...
        if ($phase) {
            $phaseText = match ($phase) {
                ...
                'calling_ai' => ' - Consulting AI',
                'classification_complete' => ' - Classification complete',
                'analyzing_request' => ' - Analyzing requirements',
                'extraction_complete' => ' - Extraction complete',
                'analyzing_requirements' => ' - Analyzing what needs to be done',
                'plan_complete' => ' - Plan ready',
                'preparing' => ' - Preparing',
                'completed' => ' - Complete',
                default => ' - ' . ucfirst(str_replace('_', ' ', $phase))
            };
            $baseMessage .= $phaseText;
        }

        return $baseMessage;
    }

    protected function isOperationComplete(string $op, string $currentOp): bool
    {
        $order = [
            'classifying' => 0,
            'extracting_tasks' => 1,
        ...
        $currentOrder = $order[$currentOp] ?? 99;
        $opOrder = $order[$op] ?? 99;

        return $opOrder < $currentOrder;
    }
