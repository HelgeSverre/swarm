...
class TUIRenderer
{
    ...
    private function drawHeader(): void
    {
        ...
        echo $this->colorize("┌─ {$title} ", 'cyan') .
            str_repeat('─', max(0, $this->terminalWidth - mb_strlen($title) - mb_strlen($time) - 8)) .
            $this->colorize(" {$time} ┐", 'cyan') . "\n";
    }

    private function drawTaskStatus(array $status): void
    {
        $tasks = $status['tasks'] ?? [];
        ...
        if ($currentTask) {
            echo $this->colorize('│ ', 'cyan') .
                ...
                str_repeat(' ', max(0, $this->terminalWidth - mb_strlen($currentTask['description']) - 25)) .
                $this->colorize('│', 'cyan') . "\n";
        }
        ...
        for ($i = 0; $i < $maxTasks; $i++) {
            ...
            echo $this->colorize('│ ', 'cyan') .
                ...
                str_repeat(' ', max(0, $this->terminalWidth - mb_strlen($task['description']) - 15)) .
                $this->colorize('│', 'cyan') . "\n";
        }
    ...
    private function drawRecentActivity(): void
    {
        ...
        if (empty($recentHistory)) {
            ...
            foreach ($recentHistory as $entry) {
                ...
                echo $this->colorize('│ ', 'cyan') .
                    ...
                    str_repeat(' ', max(0, $this->terminalWidth - mb_strlen($entry['message']) - 10)) .
                    $this->colorize('│', 'cyan') . "\n";
            }
        }
    }
    ...
    private function addToHistory(string $type, string $message, string $color = 'white'): void
    {
        $this->history[] = [
            'type' => $type,
            'message' => $message,
    ...
    private function summarizeResult($result): string
    {
        if (is_array($result)) {
    ...
    private function colorize(string $text, string $color, ?string $style = null): string
    {
        $codes = [];
        if ($style) {
